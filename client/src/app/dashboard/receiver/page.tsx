...
import { Package, Clock, HeartHandshake, Loader2, ShieldCheck, ClipboardList, Database, Send } from "lucide-react";
      ...
      <div className="min-h-screen bg-white flex flex-col font-sans">
        ...
        <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="mb-10 pb-6 border-b-[2.5px] border-gray-900 flex justify-between items-end">
            <div>
              <h1 className="text-[32px] font-normal text-gray-900 tracking-tight uppercase">Receiver Operations</h1>
              <p className="text-[16px] text-gray-600 mt-1">Manage incoming donations and logbook entries.</p>
            </div>
            <div className="bg-[#f6b26b] text-gray-900 px-4 py-1.5 font-bold tracking-widest text-[14px] uppercase shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]">
              Authorized
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Link href="/my-requests" className="flex items-center justify-between p-4 border-[1.5px] border-gray-900 bg-[#f3f4f6] hover:bg-[#4a86e8] hover:text-white transition-colors group">
                  <div className="flex items-center gap-3">
                    <Send className="w-5 h-5" />
                    <span className="font-bold uppercase tracking-wide text-[14px]">My Requests</span>
                  </div>
                  <span className="font-normal text-xl group-hover:translate-x-1 transition-transform">→</span>
                </Link>

                <Link href="/logbook" className="flex items-center justify-between p-4 border-[1.5px] border-gray-900 bg-[#f3f4f6] hover:bg-[#f6b26b] hover:text-white transition-colors group">
                  <div className="flex items-center gap-3">
                    <ClipboardList className="w-5 h-5" />
                    <span className="font-bold uppercase tracking-wide text-[14px]">My Logbooks</span>
                  </div>
                  <span className="font-normal text-xl group-hover:translate-x-1 transition-transform">→</span>
                </Link>

                <Link href="/inventory_all" className="flex items-center justify-between p-4 border-[1.5px] border-gray-900 bg-[#f3f4f6] hover:bg-[#6aa84f] hover:text-white transition-colors group">
                  <div className="flex items-center gap-3">
                    <Database className="w-5 h-5" />
                    <span className="font-bold uppercase tracking-wide text-[14px]">Global Inventory</span>
                  </div>
                  <span className="font-normal text-xl group-hover:translate-x-1 transition-transform">→</span>
                </Link>
              </div>
              ...
              <div className="border-[1.5px] border-gray-900 bg-white p-6 md:p-8">
                <div className="flex items-center justify-between border-b-[1.5px] border-gray-900 pb-4 mb-6">
                  ...
                  <ShieldCheck className="w-6 h-6 text-[#f6b26b]" />
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div>
                    <h3 className="text-[28px] font-normal text-gray-900 mb-1">{currentUser?.organization || currentUser?.name || "Partner Organization"}</h3>
                    <p className="text-[14px] font-bold text-[#f6b26b] uppercase tracking-wider">Receiver Node</p>
                  </div>

                  <Link
                    href={`/profile/${currentUser?.id}`}
                    className="px-6 py-2.5 bg-gray-900 text-white font-bold text-[14px] uppercase tracking-widest hover:bg-[#f6b26b] hover:text-gray-900 transition-colors text-center shadow-[2px_2px_0px_0px_rgba(17,24,39,0.3)]"
                  >
                    View Public Profile
                  </Link>
                </div>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-[#f3f4f6] border-[1.5px] border-gray-900 p-6 flex flex-col items-center justify-center text-center transition-transform hover:-translate-y-1">
                      ...
                      <span className="text-[32px] font-normal text-gray-900 leading-none">{stats.totalReceived}</span>
                      <span className="text-[12px] font-bold uppercase text-gray-500 mt-2 tracking-wider">Total Received</span>
                    </div>
                    ...
                    <div className="bg-[#fce8e6] border-[1.5px] border-[#cc0000] p-6 flex flex-col items-center justify-center text-center transition-transform hover:-translate-y-1">
                      <HeartHandshake className="w-8 h-8 mb-3 text-[#cc0000]" />
                      <span className="text-[32px] font-normal text-gray-900 leading-none">{stats.peopleFed.toLocaleString()}</span>
                      <span className="text-[12px] font-bold uppercase text-[#cc0000] mt-2 tracking-wider">People Fed</span>
                    </div>
                  </div>

                  {/* 4. Top Received Items Matrix */}
                  <div className="border-[1.5px] border-gray-900 bg-white p-6 md:p-8">
                    <h2 className="text-[20px] font-normal text-gray-900 uppercase tracking-widest mb-6">Top Received Commodities</h2>
