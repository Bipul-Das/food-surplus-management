...
const calculateTimeRemaining = (createdAt: string, requiredWithin: string | null) => {
  ...
  try {
    ...
    // Add required time to creation time to get target deadline
...
export default function RequestsPage() {
  ...
  // Re-fetch interval for real-time countdown updates
  ...
  useEffect(() => {
    ...
    // Update the UI every minute to keep the countdown live
  ...
  const handleConfirmPledge = async () => {
    if (!selectedDriverId) return toast.error("System Requirement: Assign Delivery Personnel to lock transaction.");
    
    const totalPledged = Object.values(pledgeAmounts).reduce((a, b) => a + b, 0);
    if (totalPledged <= 0) return toast.error("System Requirement: Invalid quantity. Enter a value greater than 0.");
    ...
    try {
      ...
      if (result.success) {
        toast.success("Transaction Locked: Logistics personnel assigned.");
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {visibleRequests.map((req) => {
                  ...
                  // Mathematical Engine for Overall Status
                  ...
                  const totalReceived = req.items.reduce((sum: number, item: any) => {
                    ...
                    return sum + Math.max(0, demanded - item.deficit);
                  }, 0);
                  
                  let overallPercent = 0;
                  if (totalDemanded > 0) {
                    overallPercent = Math.round((totalReceived / totalDemanded) * 100);
                  } else if (req.items.length > 0 && req.items.every((i: any) => i.deficit === 0)) {
                    overallPercent = 100; 
                  }

                  // Strict State Evaluations
                  const isCompleted = overallPercent >= 100;
                  const timeRemaining = calculateTimeRemaining(req.createdAt, req.requiredWithin);
                  const isExpired = timeRemaining === "Expired";
                  const isLockedOut = isCompleted || isExpired;

                  return (
                    <div key={req.id} className={`bg-white border-[3px] border-[#0a192f] p-8 flex flex-col items-center relative ${isExpired && !isCompleted ? 'opacity-70' : ''}`}>
                      
                      {/* Header Group */}
                      <div className="w-full text-center mb-8 border-b-2 border-transparent">
                      ...
                      <div className="w-full space-y-4 mb-10 px-2">
                        {req.items.map((item: any, idx: number) => {
                          ...
                          let fillPercent = 0;
                          if (demanded > 0) {
                            fillPercent = (received / demanded) * 100;
                      ...
                      <div className="w-full flex justify-center mt-auto">
                        {(user?.role === "DONOR" || user?.role === "LEAD_DEV") && (
                          <button 
                            ...
                            className={`px-10 py-3 border-[2.5px] font-bold text-[15px] transition-colors ${
                              isCompleted 
                            ...
                            <div key={agent.id} className="grid grid-cols-4 px-5 py-4 items-center text-[15px]">
                              ...
                              <div className="flex justify-end">
                                <button 
                                  ...
                                  className={`px-6 py-1.5 border-[1.5px] font-medium transition-colors ${
                                    selectedDriverId === agent.id 
